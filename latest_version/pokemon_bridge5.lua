...
-- === BADGE FLAGS (event flags 0x820-0x827 in SB1) ===       -- NEW v3.2
-- Flags base at SB1+0x0EE0, badge byte at flags_base+0x104  -- NEW v3.2
-- All 8 badges in one byte: bit0=Boulder ... bit7=Earth      -- NEW v3.2
BADGE_FLAGS_OFFSET = 0x0FE4  -- SB1 + 0x0FE4 (= 0x0EE0 + 0x104)  -- NEW v3.2
...
BADGE_UPDATE_RATE  = 60        -- NEW v3.2: badge changes at most 8 times per playthrough
...
function count_set_bits(byte)                             -- NEW v3.2
    local n = 0                                           -- NEW v3.2
    local v = byte                                        -- NEW v3.2
    while v > 0 do                                        -- NEW v3.2
        n = n + (v % 2)                                   -- NEW v3.2
        v = math.floor(v / 2)                             -- NEW v3.2
    end                                                   -- NEW v3.2
    return n                                              -- NEW v3.2
end                                                       -- NEW v3.2

function get_badge_count()                                -- NEW v3.2
    local sb1 = r32(ADDR_SB1_PTR)                        -- NEW v3.2
    if sb1 == 0 then return 0 end                         -- NEW v3.2
    local badge_byte = r8(sb1 + BADGE_FLAGS_OFFSET)       -- NEW v3.2
    return count_set_bits(badge_byte)                     -- NEW v3.2
end                                                       -- NEW v3.2
...
--   bd=badge_count (0-8)                          -- NEW v3.2
-------------------------------------------------
local cached_palette_str = ""
local cached_tiles_str = ""

function write_state(x, y, map, in_battle, game_state, direction, text_flag, badge_count, include_visual, include_bag)  -- UPDATED v3.2: added badge_count param
    ...
    f:write(string.format('],"gs":%d,"tf":%d,"bd":%d,"dead":false', game_state, text_flag, badge_count))  -- UPDATED v3.2: added "bd"
...
print("==========================================")
print("Pokemon AI v3.2 — Badge Count + Full State")            -- UPDATED v3.2
print("==========================================")
print("OVERWORLD:")
...
print("  BadgeFlags: SB1+0x0FE4 (event flags 0x820-0x827)")    -- NEW v3.2
print("  bit0=Boulder..bit7=Earth, count set bits")             -- NEW v3.2
print("MENU SYSTEM:")
...
update_bag_caches()
cached_badge_count = get_badge_count()                          -- NEW v3.2
print(string.format("SB1=0x%08X SB2_key=%d Badges=%d", sb1_cache, bag_key_cache, cached_badge_count))  -- UPDATED v3.2
...
while true do
    ...
    -- Badge cache update (very rare changes)              -- NEW v3.2
    if frame_counter % BADGE_UPDATE_RATE == 0 then         -- NEW v3.2
        cached_badge_count = get_badge_count()              -- NEW v3.2
    end                                                     -- NEW v3.2

    -- Write state                                          -- UPDATED v3.2
    write_state(x, y, map, in_battle, game_state, direction, text_flag, cached_badge_count,
    ...
    if frame_counter % 60 == 0 then
        ...
        if in_battle == 1 then
            ...
            print(string.format(
                "F:%d | %s bc:%d mc:%d | P:%d Lv%d %d/%d st:%d | E:%d Lv%d %d/%d st:%d | Menu:%d/%d PC:%d%s Bd:%d | %s",  -- UPDATED v3.2: added Bd
                frame_counter, bt_str, bc, moc,
                ps, pl, ph, r16(ADDR_P_MAXHP), pst,
                es, el, eh, r16(ADDR_E_MAXHP), est,
                mc, mm, pc, tf_str, cached_badge_count,     -- UPDATED v3.2
                action or "nil"
            ))
        else
            ...
            print(string.format(
                "F:%d | %s (%d,%d) Map:%d Dir:%d | Party:%d HP:%d/%d%s%s Bd:%d | %s",  -- UPDATED v3.2: added Bd
                frame_counter, gs_str, x, y, map, direction,
                party_count, hp, mhp, extra, tf_str, cached_badge_count,                -- UPDATED v3.2
                action or "nil"
            ))
        end
    end
